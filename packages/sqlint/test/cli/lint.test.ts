...
describe("lint", () => {
  ...
  describe("format json", () => {
    it("should get formatted message", () => {
      ...
      expect(parsed[0].filepath).toContain(
        "sqlint/test/cli/fixtures/lint/errorSql.sql"
      );
  ...
  describe("fix", () => {
    it("should be fixed correctly. case1", () => {
      const sql =
        "SELECT employees.first_name, employees.email, e.first_name, e.department_id, e.manager_id, e.hire_date" +
        " FROM employees e" +
